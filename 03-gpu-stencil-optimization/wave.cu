...
std::pair<float *, float *> wave_gpu_naive(
    ...
    dim3 num_blocks = dim3((n_cells_x + block_size.x - 1) / block_size.x,(n_cells_y + block_size.y - 1) / block_size.y);
...
constexpr int S = 1; // steps per launch
    ...
    float *out1)
{
    ...
    // Same as your code but tile is bigger by S on each side
    constexpr int tw = BLOCK_X + 2 * S;
    constexpr int th = BLOCK_Y + 2 * S;

    // Two buffers in shmem, same as your original idea
    float *sh_u0 = shmem;
    float *sh_u1 = shmem + tw * th;

    int tx = threadIdx.x;
    int ty = threadIdx.y;

    // Global coords of top-left of halo (shifted by S vs your original)
    int gx_base = blockIdx.x * BLOCK_X - S;
    int gy_base = blockIdx.y * BLOCK_Y - S;

    auto sh_idx = [&](int x, int y) { return y * tw + x; };

    // --- Load full tile including halo (your loading logic, extended) ---
    // Each thread loads multiple cells to cover the bigger TW x TH tile
    int tid = ty * BLOCK_X + tx;
    int block_threads = BLOCK_X * BLOCK_Y;

    for (int i = tid; i < tw * th; i += block_threads) {
        int lx = i % tw;
        int ly = i / tw;
        int gx = gx_base + lx;
        int gy = gy_base + ly;
        ...
        if (gx >= 0 && gx < n_cells_x && gy >= 0 && gy < n_cells_y) {
            v0 = u0[gy * n_cells_x + gx];
            v1 = u1[gy * n_cells_x + gx];
        }
        sh_u0[sh_idx(lx, ly)] = v0;
        sh_u1[sh_idx(lx, ly)] = v1;
    }

    __syncthreads();

    // --- Your same stencil logic, but looped S times in shmem ---
    for (int step = 0; step < n_steps; ++step) {
        float t = t0 + step * dt;

        // This thread's fixed local coords in the tile
        int lx = tx + S;
        int ly = ty + S;
        int gx = gx_base + lx;
        int gy = gy_base + ly;
        ...
        float next = 0.f;
        bool in_grid = (gx >= 0 && gx < n_cells_x && gy >= 0 && gy < n_cells_y);

        if (in_grid) {
            bool is_border = (gx == 0 || gx == n_cells_x-1 ||
                              gy == 0 || gy == n_cells_y-1);

            // Exactly your stencil logic
            if (is_border || Scene::is_wall(gx, gy)) {
                next = 0.f;
            } else if (Scene::is_source(gx, gy)) {
                next = Scene::source_value(gx, gy, t);
            } else {
                float damping = Scene::damping(gx, gy);
                next = (2.f - damping - 4.f * coeff) * sh_u1[sh_idx(lx,   ly  )]
                     - (1.f - damping)               * sh_u0[sh_idx(lx,   ly  )]
                     + coeff * (sh_u1[sh_idx(lx-1, ly  )] +
                                sh_u1[sh_idx(lx+1, ly  )] +
                                sh_u1[sh_idx(lx,   ly-1)] +
                                sh_u1[sh_idx(lx,   ly+1)]);
            }
        }

        // Wait for all reads to finish before anyone writes
        __syncthreads();

        if (in_grid) {
            sh_u0[sh_idx(lx, ly)] = sh_u1[sh_idx(lx, ly)]; // old u1 -> new u0
            sh_u1[sh_idx(lx, ly)] = next;                   // new u1
        }

        // Wait for all writes to finish before next step reads
        __syncthreads();
    }

    // --- Write back center BLOCK_X x BLOCK_Y region (your same writeback logic) ---
    int gx = gx_base + S + tx;
    int gy = gy_base + S + ty;
    if (gx >= 0 && gx < n_cells_x && gy >= 0 && gy < n_cells_y) {
        int g = gy * n_cells_x + gx;
        out0[g] = sh_u1[sh_idx(tx + S, ty + S)]; // newest: u(t+dt)
        out1[g] = sh_u0[sh_idx(tx + S, ty + S)]; // previous: u(t)
    }
}
...
std::pair<float*, float*> wave_gpu_shmem(
    float t0, int32_t n_steps,
    float *u0, float *u1,
    float *extra0, float *extra1)
{
    dim3 block(BLOCK_X, BLOCK_Y);
    dim3 grid((Scene::n_cells_x + BLOCK_X - 1) / BLOCK_X,
              (Scene::n_cells_y + BLOCK_Y - 1) / BLOCK_Y);

    constexpr int tw = BLOCK_X + 2 * S;
    constexpr int th = BLOCK_Y + 2 * S;
    size_t shmem_bytes = 2 * tw * th * sizeof(float); // sh_u0 + sh_u1

    float *in0 = u0, *in1 = u1, *out0 = extra0, *out1 = extra1;
    float *bufs[4] = {u0, u1, extra0, extra1};

    // Your same loop, just steps S at a time instead of 1
    int remaining = n_steps;
    float t = t0;
    while (remaining > 0) {
        int steps_this_launch = min(remaining, S);

        wave_gpu_shmem_multistep<Scene><<<grid, block, shmem_bytes>>>(
            t, steps_this_launch, in0, in1, out0, out1);

        t += steps_this_launch * Scene::dt;
        remaining -= steps_this_launch;

        // Your same buffer rotation logic
        float *new_in0 = out0, *new_in1 = out1;
        float *new_out0 = nullptr, *new_out1 = nullptr;
        for (auto b : bufs) {
            if (b != new_in0 && b != new_in1) {
                if (!new_out0) new_out0 = b;
                else           new_out1 = b;
            }
        }
        in0 = new_in0; in1 = new_in1;
        out0 = new_out0; out1 = new_out1;
    }
