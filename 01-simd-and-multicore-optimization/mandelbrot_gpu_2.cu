...
__global__ void mandelbrot_gpu_vector_ilp(
    ...
    constexpr int ILP = 4;   // independent pixels per thread

    int tid = threadIdx.x;   // 0..31
    int warp_width = 32 * ILP;
    int start = tid * ILP;
    int total_pixels = img_size * img_size;

    // Loop over the image in vector-sized chunks
    for (int base = 0; base < total_pixels; base += warp_width) {

        int base_pixel = base + start;
...
__global__ void mandelbrot_gpu_vector_multicore(
    ...
    int total_threads = gridDim.x * blockDim.x; // total threads launched

    int total_pixels = img_size * img_size;

    // Stride loop over pixels so each thread handles multiple pixels
    for (int pixel = global_tid; pixel < total_pixels; pixel += total_threads) {
        int row = pixel / img_size;
        int col = pixel % img_size;

        // Map pixel to complex plane
        float cx = float(col) / img_size * window_zoom + window_x;
        float cy = float(row) / img_size * window_zoom + window_y;
...
void launch_mandelbrot_gpu_vector_multicore(
    ...
    mandelbrot_gpu_vector_multicore<<<blocks, threads>>>(img_size, max_iters, out);
...
__global__ void mandelbrot_gpu_vector_multicore_multithread_single_sm(
    ...
    int thread_id = threadIdx.x;         // Unique thread in block
    int total_threads = blockDim.x;      // Total threads in this block
    uint32_t total_pixels = img_size * img_size;

    // Each thread processes every total_threads-th pixel
    for (uint32_t pixel = thread_id; pixel < total_pixels; pixel += total_threads) {
        int row = pixel / img_size;
        int col = pixel % img_size;
...
void launch_mandelbrot_gpu_vector_multicore_multithread_single_sm(
    ...
    int num_warps = 8; // can experiment: 4 → 32
...
__global__ void mandelbrot_gpu_vector_multicore_multithread_full(
    ...
    int global_tid = block_offset + thread_id;

    int total_threads = gridDim.x * blockDim.x;
    uint32_t total_pixels = img_size * img_size;

    for (uint32_t pixel = global_tid; pixel < total_pixels; pixel += total_threads) {
        int row = pixel / img_size;
        int col = pixel % img_size;
...
void launch_mandelbrot_gpu_vector_multicore_multithread_full(
    ...
    int warps_per_block = 8;  // try 4 → 32
    int threads_per_block = warps_per_block * 32;
    
    int num_sms = 48;          // adjust to your GPU
    int blocks = num_sms;      // 1 block per SM minimum

    mandelbrot_gpu_vector_multicore_multithread_full<<<blocks, threads_per_block>>>(
        img_size, max_iters, out);
}
...
__global__ void mandelbrot_gpu_vector_multicore_multithread_full_ilp(
    ...
    int thread_index = blockIdx.x * threads_per_block + tid;

    int total_pixels = img_size * img_size;

    // Loop over the image in vector-sized chunks across all threads
    for (int base = thread_index * ILP; base < total_pixels; base += total_threads * ILP) {
        float cx[ILP], cy[ILP];
        float x2[ILP], y2[ILP], w[ILP];
        uint32_t iters[ILP];
        bool active[ILP];
...
void launch_mandelbrot_gpu_vector_multicore_multithread_full_ilp(
    ...
    int threads_per_block = 128;         // multiple of 32 for warp alignment
    int num_blocks = 48;                 // roughly one per SM, adjust as needed
